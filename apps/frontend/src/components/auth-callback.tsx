...
import { AuthErrorMap, getAuthErrorMessage } from "@/lib/auth-errors";
...
export default function AuthCallback() {
  ...
  const [callbackType, setCallbackType] = useState<AuthCallbackType | null>(
    null
  );
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-[50vh] max-w-md mx-auto text-center px-4">
      ...
      <p className="text-sm text-muted-foreground mb-8">
        Redirecting in{" "}
        <span className="font-bold text-foreground">{seconds}</span> seconds
      </p>
