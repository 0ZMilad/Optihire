...
export function AuditResultsView({
  ...
  const jdFirstLine = useMemo(
    () => extractFirstLine(context.jobDescription),
    [context.jobDescription]
  );

  // AI enhancement payload (opt-in career coach)
  const aiEnhancement = result.ai_enhancement ?? null;
  const aiProviderLabel = aiEnhancement?.provider_label?.trim() || "AI";
  ...
  const checklistItems = useMemo(
    () => [
      ...
      },
      {
        ...
        fixHint:
          "Keep most lines under 100 characters — long lines can confuse ATS parsers.",
      },
      {
      ...
      },
      {
        ...
        fixHint:
          "Use bullet points and keep lines under 100 characters for reliable ATS scanning.",
      },
    ],
    [
      ...
      result.has_skills,
      result.has_consistent_formatting,
      result.has_bullet_points,
      result.has_action_verbs,
      result.is_scannable,
    ]
  );

  const checklistPassCount = useMemo(
    () => checklistItems.filter((i) => i.passed).length,
    [checklistItems]
  );
  const criticalCount = useMemo(
    () => priorityActions.filter((a) => a.priority === "critical").length,
    [priorityActions]
  );
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-1.5 shrink-0">
          ...
          >
            ...
            {jdFirstLine
              ? jdFirstLine.slice(0, 30) + (jdFirstLine.length > 30 ? "…" : "")
              : "Job Description"}
            {jdExpanded ? (
      ...
      <div
        ...
        className="flex items-center gap-1 overflow-x-auto pb-0.5 scrollbar-none border-b"
      >
        {tabs.map((tab) => (
          <button
            key={tab.id}
