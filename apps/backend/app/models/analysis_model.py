...
from sqlalchemy import func, Text, ARRAY, String, UniqueConstraint, Index, CheckConstraint
...
class AnalysisResult(SQLModel, table=True):
    ...
    __table_args__ = (
        ...
        CheckConstraint("keyword_score >= 0 AND keyword_score <= 100", name="chk_keyword_score"),
        CheckConstraint("formatting_score >= 0 AND formatting_score <= 100", name="chk_formatting_score"),
        CheckConstraint("section_score >= 0 AND section_score <= 100", name="chk_section_score"),
    ...
    resume_content_hash: Optional[str] = Field(default=None, max_length=64)
    job_description_hash: Optional[str] = Field(default=None, max_length=64)
    ...
    keyword_density: Optional[Decimal] = Field(default=None, max_digits=6, decimal_places=3)
    ...
    suggestions_payload: Optional[dict] = Field(default=None, sa_column=Column(JSONB))
    skills_version: Optional[str] = Field(default=None, max_length=20)
    keywords_rules_version: Optional[str] = Field(default=None, max_length=20)
...
class Suggestion(SQLModel, table=True):
    ...
    __table_args__ = (
        CheckConstraint("state IN ('suggested', 'viewed', 'acted_upon', 'dismissed')", name="chk_suggestion_state"),
        CheckConstraint("impact_level IN ('high', 'medium', 'low')", name="chk_suggestion_impact"),
...
class SkillCorrection(SQLModel, table=True):
    ...
    __table_args__ = (
        Index("idx_skill_corrections_resume", "resume_id"),
    )
...
class IndustryKeyword(SQLModel, table=True):
    ...
    __table_args__ = (
        CheckConstraint("importance_score >= 0 AND importance_score <= 100", name="chk_importance_score"),
        UniqueConstraint(
            "industry",
            "keyword",
            "job_category",
    ...
    job_category: Optional[str] = Field(default=None, max_length=100)
    keyword: str = Field(max_length=100, nullable=False)
    keyword_type: Optional[str] = Field(default=None, max_length=50)
